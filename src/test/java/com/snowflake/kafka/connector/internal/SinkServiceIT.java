...
public class SinkServiceIT {
  ...
  private void insert(SnowflakeSinkService sink, int partition, int numOfRecord) {
    for (int i = 0; i < numOfRecord; i++) {
      ...
      sink.insert(
          new SinkRecord(
              topic, partition, Schema.STRING_SCHEMA, "test", input.schema(), input.value(), i));
    }
  }
  ...
  public void testBrokenRecord() {
    ...
    SnowflakeConverter converter = new SnowflakeJsonConverter();
    SchemaAndValue result =
        converter.toConnectData(topic, "as12321".getBytes(StandardCharsets.UTF_8));
    SinkRecord record =
        new SinkRecord(
            topic,
            partition,
  ...
  public void testCleanerRecover(boolean useMultipleTopicsToTable) throws Exception {
    ...
    doThrow(SnowflakeErrors.ERROR_2001.getException())
        .when(spyConn)
        .purgeStage(anyString(), anyList());
    // Sleep 6 minutes so that cleaner encounters 6 exceptions. Just to make sure cleaner restart is
    // triggered
  ...
  /**
   * This test is ignored because it is tested manually. Need to check snowflake query history to
   * verify that there are only list stage commands every minutes for each pipe.
   */
  @Ignore
  @Test
